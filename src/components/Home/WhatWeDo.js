      ...
      <div className="w-full grid sm:grid-cols-2 gap-6">
        {whatWeDo.map((data,id) => (
          <Link key={id} href={data.href}>
            <div
              className={`h-full flex items-center gap-2 sm:gap-6 p-2 sm:p-4 rounded-2xl border-2 border-b-8 border-black hover:scale-102 transition flex-row sm:flex-col lg:flex-row cursor-pointer ${
                data.bg == "black"
