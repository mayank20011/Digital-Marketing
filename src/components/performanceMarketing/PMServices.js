  ...
  return (
    <div>
      <div className="custom-container flex flex-col md:flex-row gap-6 md:gap-12 items-center">
        <div className="w-full md:w-1/2 flex flex-col gap-6 psImgContainer">
          <Image
            src={"/pmServices.png"}
            alt={"graph Image"}
            width={500}
            height={500}
            className="w-full drop-shadow-[2px_2px_2px_black] psimage relative"
          />
          <h1 className="text-4xl font-bold relative">{data.heading}</h1>
          <p className="relative">{data.pera}</p>
          <Link
            href={"/contact-us"}
            className="bg-gradient-to-r from-orange-400 to-pink-400 w-fit px-4 py-2 rounded-full border shadow-[3px_3px_1px_black] hover:-translate-y-[5px] transition hover:shadow-[5px_5px_3px_black] relative"
          >
            Contact Us
          </Link>
        </div>
        <div className="w-full md:w-1/2">
          {data.cards.map((card, id) => (
            <div
              ...
              } ${id == data.cards.length - 1 ? "border-b-1" : ""} relative pmCardContainer overflowx-x-hidden`}
            >
              <div
